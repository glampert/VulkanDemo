...
private:
    ...
    const char *             m_shaderFilename = VKTB_SHADER_SOURCE_PATH "TexturedSimple.glsl";
    ...
    , m_sharedSampler{ context() }
{
    ...
    clearValue.color.float32[0] = 0.7f;
    clearValue.color.float32[1] = 0.7f;
    clearValue.color.float32[2] = 0.7f;
    clearValue.color.float32[3] = 1.0f;
    context().setDefaultClearValue(clearValue);
}
...
void VkAppTexturedCubes::updateBuffers(CommandBuffer & cmdBuff)
{
    ...
    m_matrices[0].model      = Matrix4::translation(Vector3{ -1.0f, 0.0f, -1.0f }) * Matrix4::rotationZYX(Vector3{ 0.0f, rotationY * DegToRad, rotationX * DegToRad });
    m_matrices[0].view       = Matrix4::lookAt(Point3{ 0.0f, 0.0f, 3.0f }, Point3{ 0.0f, 0.0f, 0.0f }, Vector3{ 0.0f, 1.0f, 0.0f });
    m_matrices[0].projection = Matrix4::perspective(45.0f * DegToRad, context().framebufferAspect(), 0.1f, 10.0f);
    m_matrices[0].mvp        = m_matrices[0].projection * m_matrices[0].view * m_matrices[0].model;
    m_uniformBuffer.write(m_matrices[0], 0);

    m_matrices[1].model      = Matrix4::translation(Vector3{ 1.0f, 0.0f, -1.0f }) * Matrix4::rotationZYX(Vector3{ 0.0f, rotationY * DegToRad, rotationX * DegToRad });
    m_matrices[1].view       = Matrix4::lookAt(Point3{ 0.0f, 0.0f, 3.0f }, Point3{ 0.0f, 0.0f, 0.0f }, Vector3{ 0.0f, 1.0f, 0.0f });
    m_matrices[1].projection = Matrix4::perspective(45.0f * DegToRad, context().framebufferAspect(), 0.1f, 10.0f);
